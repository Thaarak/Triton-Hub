...
def call_llm(user_text: str) -> str:
    ...
    # Use a model that works with Google AI Studio (try in order)
    # Note: Free tier rate limits are often shared, but we try anyway.
    for model_id in ("gemini-2.0-flash", "gemini-2.5-flash", "gemini-1.5-flash"):
        try:
            response = generate_with_retry(model_id, user_text)
            return (response.text or "").strip()
        except Exception as e:
            # If 404 (model not found), try next model.
            # If 429 (rate limit), we already retried inside generate_with_retry, 
            # so if it still fails, it bubbles up. We catch it here to try the next model
            # just in case a different model has quota.
            err_str = str(e).lower()
            if "404" in err_str or "not found" in err_str:
                continue
            if "429" in err_str or "resource_exhausted" in err_str:
                print(f"Model {model_id} hit rate limit after retries. Trying next model...", file=sys.stderr)
                continue
            raise
            
    raise SystemExit("Failed to generate content. Check your API key and Quotas at https://aistudio.google.com/app/apikey")
...
def upload_to_supabase(notifications: list[dict]) -> bool:
    ...
    try:
        ...
        print("Checking for duplicates and similar items in Supabase...", file=sys.stderr)
        existing_notifications = check_existing_notifications(supabase)
        ...
        if not unique_notifications:
            print("No new notifications to upload (all were duplicates/similar or input was empty).", file=sys.stderr)
            return True
